...
from prompts import templete1,template_cls
            ...
            if all(d["instruction"] in existing_requests for d in batch):
                ...
                for task in batch:
                    ...
                    try:
                        ...
                        print(model_result)
                        data = task
                        pattern = re.compile(r'Input:(.*?)Class Labels:(.*?)(Input:|$)', re.DOTALL)
                        matches = pattern.search(model_result)
                        data["input"]=matches.group(1).strip()
                        data["output"]="Class Labels:"+matches.group(2).strip()
