...
def handle_completion(comp, num):
    ...
    if len(comp.choices) == 1:
        ...
        raw = re.findall(r'\s*Task\s*\d+\.\s*(.*)', cho)
        ret = []
        for ins in raw:
    ...
    while len(machine_instructions) < num_instructions_to_generate:
        with open(r"D:\Desktop\dsa\machine_tasks-{}.jsonl".format(int((len(machine_instructions) + 999)/1000 + ((len(machine_instructions) % 1000) == 0))), "a") as fw:
            ...
            for tt in range(round):
                ...
                prompt = "Come up with {} tasks. {} tasks are provided, and you need to propose {} different tasks. You don't need to solve these tasks.\n".format(machine_sample + seed_sample + machine_gen, machine_sample + seed_sample, machine_gen)
